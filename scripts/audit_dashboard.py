...
def main():
 ...
 labels=("Published Release","Upcoming","公開予定","MV不足","News不足","Gallery不足","Wiki不足","Universe不足","画像不足","SEO不足","JSON-LD不足","Search Console登録候補","YouTube未設定","Instagram未設定","公開日時未設定","おすすめ未設定","Published Lyrics","Verified Lyrics waiting for release","Unresolved Lyrics","次回公開予定")
 for x in labels:
  if x not in text: errors.append(f"missing {x}")
 if 'noindex, nofollow' not in text: errors.append("dashboard must be noindex")
 if "creator-cms.json" not in js or "recommendations.json" not in js or "dashboard-status.json" not in js: errors.append("dashboard is not source-backed")
