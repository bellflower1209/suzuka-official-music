...
def gallery_pages(root: Path, releases: list[dict], release_links: dict) -> None:
    ...
    for position, item in enumerate(gallery_releases):
        ...
        adjacent = []
        if position > 0:
            previous = gallery_releases[position - 1]
            adjacent.append(
                f'<a href="../{previous["slug"]}/" rel="prev">前のVisual Archive：{html.escape(previous["displayTitle"])}</a>'
            )
        if position + 1 < len(gallery_releases):
            following = gallery_releases[position + 1]
            adjacent.append(
                f'<a href="../{following["slug"]}/" rel="next">次のVisual Archive：{html.escape(following["displayTitle"])}</a>'
            )
        adjacent_links = "".join(adjacent)
        genres = " / ".join(item.get("genres", [])) or "正本ジャンル未設定"
        ...
        body = (
            ...
            '<section class="explorer-archive-facts" aria-labelledby="visual-archive-data">'
            '<p class="section-kicker">VERIFIED ARCHIVE DATA</p><h2 id="visual-archive-data">Visual Archive情報</h2>'
            '<p>このページは楽曲情報をまとめるReleaseとは役割を分け、公開確認済みのMVと画像資料を記録するVisual Archiveです。</p>'
            '<dl>'
            f'<div><dt>作品</dt><dd>{html.escape(item["displayTitle"])}</dd></div>'
            f'<div><dt>アーティスト</dt><dd><a href="../../artists/{html.escape(item["artistSlug"])}/">{html.escape(item["artist"])}</a></dd></div>'
            f'<div><dt>公開日</dt><dd><time datetime="{item["releaseDate"]}">{item["releaseDate"]}</time></dd></div>'
            f'<div><dt>ジャンル</dt><dd>{html.escape(genres)}</dd></div>'
            f'<div><dt>映像</dt><dd>Official MV（YouTube ID: {html.escape(youtube_id)}）</dd></div>'
            '<div><dt>収録資料</dt><dd>公式ジャケット / YouTubeサムネイル / Official MV</dd></div>'
            '</dl></section>'
            f'<nav class="explorer-gallery-adjacent" aria-label="前後のVisual Archive">{adjacent_links}</nav>'
    ...
    write(
        ...
        shell(
            ...
            "MV GALLERY", body,
            [{"@type": "ItemList", "numberOfItems": len(gallery_releases), "itemListElement": elements}],
        ),
    )
...
.explorer-archive-facts{margin:0 clamp(1.25rem,6vw,7rem) 3rem;padding:clamp(1.5rem,4vw,2.5rem);border:1px solid rgba(255,255,255,.14);border-radius:1rem;background:rgba(255,255,255,.035)}.explorer-archive-facts>p{max-width:58rem;color:#c8c0cf}.explorer-archive-facts dl{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.75rem;margin:1.5rem 0 0}.explorer-archive-facts dl>div{padding:1rem;border-top:1px solid rgba(255,255,255,.12)}.explorer-archive-facts dt{color:#a9a1ae;font-size:.75rem}.explorer-archive-facts dd{margin:.35rem 0 0;color:#fff;overflow-wrap:anywhere}.explorer-archive-facts a{color:#fff}.explorer-gallery-adjacent{display:flex;justify-content:space-between;gap:1rem;padding:0 clamp(1.25rem,6vw,7rem) 5rem}.explorer-gallery-adjacent a{color:#fff;text-underline-offset:.2em;overflow-wrap:anywhere}
...
@media(max-width:760px){.explorer-hero{padding-top:6.3rem}.explorer-card-grid,.explorer-ranking-grid,.explorer-feature-grid,.explorer-wiki-grid,.explorer-news-grid,.explorer-home-portals{grid-template-columns:1fr}.explorer-gallery-grid{grid-template-columns:repeat(2,minmax(0,1fr));padding-left:1rem;padding-right:1rem}.explorer-gallery-detail,.explorer-artist-hub-hero,.explorer-artist-profile{grid-template-columns:1fr}.explorer-production-note>div,.explorer-archive-facts dl{grid-template-columns:1fr}.explorer-gallery-adjacent{flex-direction:column}.explorer-universe-map{grid-template-columns:1fr 1fr}.explorer-universe-artist{grid-template-columns:1fr}.explorer-universe-artist>img{max-width:15rem}.explorer-universe-work{grid-column:auto;max-width:14rem}.explorer-artist-hub-hero{padding-top:5rem}.explorer-artist-hub-hero>img{grid-row:1}.explorer-artist-works article{grid-template-columns:4.5rem 4.5rem 1fr}.explorer-artist-works img{width:4.5rem}.explorer-artist-works article>a{grid-column:3}.explorer-home-heading{align-items:start;flex-direction:column}.explorer-feature-grid{padding-left:1rem;padding-right:1rem}}
