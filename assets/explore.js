(() => {
  ...
  const card = item => `<article class="explore-card"><img src="${link(item.coverImage)}" alt="${esc(item.coverAlt)}" width="1280" height="720" loading="lazy"><div class="explore-card-copy"><time datetime="${item.releaseDate}">${esc(item.releaseDate.replaceAll("-","."))}</time><h2>${esc(item.displayTitle)}</h2><p>${esc(item.artist)}</p><div class="explore-tags">${item.genres.map(g=>`<span class="explore-tag">${esc(g)}</span>`).join("")}</div>${item.streamingRelease?`<p>STREAMING RELEASE · ${esc(item.streamingRelease.releaseDate.replaceAll("-","."))}</p>`:""}<div class="explore-actions">${item.streamingRelease?`<a href="${esc(item.streamingRelease.linkcoreUrl)}" target="_blank" rel="noopener noreferrer">配信サービスで聴く ↗</a>`:""}<a href="${link(item.releaseUrl)}">作品ページ</a><a href="${esc(item.youtubeUrl)}" target="_blank" rel="noopener noreferrer">公式MV ↗</a>${item.newsUrl?`<a href="${link(item.newsUrl)}">News</a>`:""}</div></div></article>`;
  fetch(catalogUrl).then(r => {
    ...
    const update=({push=false}={})=>{
      ...
      let items=published.filter(i=>(!needle||normalize([i.title,i.displayTitle,i.artist,...(i.genres||[]),...(i.moods||[]),...(i.themes||[]),...(i.tags||[]),...(i.searchKeywords||[]),i.lyricsText,i.lyricist,i.composer,i.introduction,i.description,i.releaseYear,i.releaseType].join(" ")).includes(needle))&&(!artist.value||i.artistSlugs.includes(artist.value))&&(!genre.value||i.genres.includes(genre.value))&&(!theme.value||(i.themes||[]).includes(theme.value))&&(!year.value||String(i.releaseYear)===year.value)&&(!type.value||(i.releaseType||"single")===type.value));
